...
def parse_args(*args, **kwargs):
    ...
    p_group.add_argument('-i','--init', help="Initialize a local sqlite3 db in /home/$USER/.config/jc path", action="store_true")
...
def init(cursor):
    try:
        cursor.execute("""CREATE TABLE main(environment text,hostname text,ip_address real,username text,password text)""")
    except Exception as exc:
        print('{} *> {}{}{}'.format(BOLD,WARNING,exc,ENDC))
...
def delete(cursor):
    ...
    print("{} *> '{}' drop(s) {}successful{}".format(BOLD,len(inpt), OKGREEN, ENDC))
